// 09.04.2026 16:20 - LEGAL: Updated terms to reflect "Privacy by Design", Serverless PWA Architecture and removed contradicting usage rights.
/**
...
Nutzer müssen mindestens 18 Jahre alt sein, um die Google Gemini API über das Programm zu nutzen.
...
Das Programm ist konsequent nach dem Prinzip „Privacy by Design“ als Progressive Web App (PWA) ohne eigene Server-Infrastruktur konzipiert. Der Anbieter selbst erhebt, speichert oder verarbeitet **keinerlei** personenbezogene Daten, API-Keys oder eingegebene Reiseinformationen des Nutzers. Sämtliche Eingaben, Konfigurationen und generierten Reisepläne verbleiben ausschließlich lokal auf dem Endgerät des Nutzers (im lokalen Speicher des Browsers). Die Übermittlung der Daten zur Generierung der Pläne erfolgt direkt vom Endgerät des Nutzers an die Server von Google (siehe 4.2).

**4.2. Datenverarbeitung durch Google Gemini API**
Das Programm übermittelt die vom Nutzer eingegebenen Daten (Prompts) an die Google Gemini API.
...
Da der Anbieter selbst keine Daten verarbeitet (siehe 4.1), können sich Auskunftsersuchen nur auf die lokale Browser-Speicherung beziehen, über die der Nutzer ohnehin selbst die volle Kontrolle hat.

**V. Geistiges Eigentum und Nutzungsrechte**

**5.1. Urheberschaft an KI-generierten Inhalten**
Nach deutschem Recht entsteht an rein maschinell (durch KI) erstellten Inhalten in der Regel kein Urheberrecht; sie gelten als gemeinfrei. Greift der Nutzer durch gezielte und kreative Eingaben (Prompts, Notizen) wesentlich in den Entstehungsprozess ein, kann er als Urheber der so geschaffenen Werke gelten.

**5.2. Verzicht auf Nutzungsrechte durch den Anbieter**
Konform mit dem „Privacy by Design“-Ansatz operiert die Software rein lokal. Da der Anbieter technisch keinen Zugriff auf die Daten hat, erhebt er **keinerlei Ansprüche, Eigentums- oder Nutzungsrechte** an den eingegebenen Daten oder den generierten Reiseplänen. Der Nutzer kann die mit Papa-Tours erstellten Reisepläne für private und kommerzielle Zwecke frei verwenden, sofern dies nicht gegen die AGB von Google verstößt.
