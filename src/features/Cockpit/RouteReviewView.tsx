...
import { ItineraryModal } from './ItineraryModal'; // NEU: Import

// NEU: Props Interface für Navigation
...
export const RouteReviewView: React.FC<RouteReviewViewProps> = ({ onNext }) => {
  ...
  const handleConfirmSelection = () => {
    ...
    // Note: RouteProposal uses "uebernachtungsorte" (strings), we need objects
    // We assume the route architect returns valid names.
    const stops = selectedRoute.uebernachtungsorte.map((loc, i) => ({
      id: `stop-${Date.now()}-${i}`,
      location: loc,
      duration: 1, // Default duration, will be edited in Modal
      hotel: undefined // Reset hotel as route changed
    }));

    // 2. Update Store
    updateLogistics('roundtrip', {
      stops: stops,
      // Optional: Set Trip Mode to 'fix' if needed
    });

    // 3. Open Itinerary Modal (statt nur Notification)
    setShowItineraryModal(true);
  };

  const handleItinerarySaved = () => {
    setShowItineraryModal(false);
    // Trigger parent navigation (z.B. Wizard Next Step)
    if (onNext) onNext();
  };
  ...
  // Determine Anchor Point for Maps (Start Location)
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* NEU: Itinerary Modal Integration */}
      <ItineraryModal 
