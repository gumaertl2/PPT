...
export const SightCard: React.FC<SightCardProps> = ({ id, data, mode = 'selection', showPriorityControls = true }) => {
  ...
  const specialDuration = data.details?.duration; 
  const specialNote = data.details?.note;
  const websiteUrl = data.website || data.details?.website;
  const sourceUrl = data.source_url; // NEW: Guide Source Link
      ...
      <div ref={cardRef} className={`bg-white rounded-lg shadow-sm border p-3 mb-3 transition-all hover:shadow-md ${borderClass}`}>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-600 mb-1">
            ...
            <div className="flex items-center gap-2 ml-auto no-print">
               ...
               <a href={`https://www.google.com/search?q=${encodeURIComponent(name + ' ' + (data.city || ''))}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-600"><Search className="w-3.5 h-3.5" /></a>
               <button onClick={(e) => { e.stopPropagation(); setUIState({ selectedPlaceId: id, viewMode: 'map' }); }} className="text-gray-400 hover:text-blue-600 transition-colors"><MapIcon className="w-3.5 h-3.5" /></button>
            </div>
        </div>
        {isFixed && (
          <div className="flex items-center gap-2 bg-indigo-50 px-2 py-1 rounded text-[10px] mb-1">
            ...
            <input type="date" value={fixedDate} onChange={(e) => handleDateChange('fixedDate', e.target.value)} className="bg-transparent border-none p-0 h-4 text-indigo-900 focus:ring-0" />
            <input type="time" value={fixedTime} onChange={(e) => handleDateChange('fixedTime', e.target.value)} className="bg-transparent border-none p-0 h-4 text-indigo-900 focus:ring-0" />
          </div>
        )}
          ...
          <div className="text-sm text-gray-600 mt-2 pt-2 border-t border-gray-100 animate-in fade-in duration-200">
            ...
            {signatureDish && (
                <div className="flex items-start gap-1.5 mb-2 text-[11px] text-amber-800 bg-amber-50 p-1.5 rounded border border-amber-100">
                    ...
                    <span className="font-medium italic">"{highlightText(signatureDish)}"</span>
                </div>
            )}

            {(cuisine || vibe.length > 0) && (
               <div className="flex flex-wrap gap-1 mb-2 text-[10px]">
                  {cuisine && (<span className="inline-flex items-center gap-1 bg-orange-50 text-orange-800 px-2 py-0.5 rounded border border-orange-100"><Utensils className="w-3 h-3" /> {cuisine}</span>)}
                  {vibe.map((v: string, i: number) => (<span key={i} className="inline-flex items-center gap-1 bg-purple-50 text-purple-800 px-2 py-0.5 rounded border border-purple-100"><Sparkles className="w-3 h-3" /> {v}</span>))}
               </div>
            )}
            <p className={`${isDetailed ? '' : 'line-clamp-2'} leading-snug text-xs mb-2`}>{highlightText(description)}</p>
            ...
            {isSpecial && specialNote && (<p className="text-[10px] text-amber-700 italic mb-2 border-l-2 border-amber-300 pl-2 leading-tight bg-amber-50 p-1 rounded-r"><span className="font-bold not-italic">Tipp: </span>"{highlightText(specialNote)}"</p>)}
            {awards.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2 mt-1">
                {awards.map((award: string, i: number) => (<span key={i} className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full shadow-sm"><Trophy className="w-3 h-3 text-amber-600 fill-amber-100" />{highlightText(award)}</span>))}
              </div>
            )}
            <div className="text-xs text-gray-500 leading-snug space-y-1">
               <div className="flex flex-wrap gap-x-3 gap-y-1">
                   ...
                   {phone && (<span className="flex items-center gap-1 text-gray-600"><Phone className="w-3 h-3 text-gray-400" /><a href={`tel:${phone}`} className="hover:text-blue-600 hover:underline">{phone}</a></span>)}
               </div>
               <div className="flex flex-wrap gap-x-3 gap-y-1">
                   {openingHoursHint && (<span className="flex items-center gap-1"><span className="font-semibold text-gray-400">🕒</span> {highlightText(openingHoursHint)}</span>)}
            ...
            {/* FIX: Logistics Mapping (Supports standard logistics AND Enricher Tip) */}
            {logistics && (
                <div className="mt-1.5 bg-slate-50 p-1.5 rounded text-[11px] text-slate-700 border border-slate-100 leading-tight">
                    <span className="font-bold text-slate-500 mr-1">Logistik:</span>{highlightText(logistics)}
                </div>
            )}
