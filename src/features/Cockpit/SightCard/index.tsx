...
const normalizeLevel = (lvl: string | undefined): ViewLevel => {
    if (lvl === 'kompakt' || lvl === 'compact') return 'compact';
    if (lvl === 'standard') return 'standard';
    if (lvl === 'details') return 'details';
    return 'compact';
};
...
export const SightCard: React.FC<SightCardProps> = ({ 
  ...
  const getEffectiveViewLevel = (): ViewLevel => {
      if (overrideDetailLevel) return normalizeLevel(overrideDetailLevel as string);
      if (uiState.isPrintMode && uiState.printConfig?.detailLevel) {
          return normalizeLevel(uiState.printConfig.detailLevel as string);
      }
      return normalizeLevel(uiState.detailLevel as string);
  };
  ...
  const effectiveCategory = userSelection.customCategory || category;
  const isHotel = effectiveCategory.toLowerCase() === 'hotel' || effectiveCategory.toLowerCase() === 'accommodation';
  const isSpecial = effectiveCategory === 'special';
